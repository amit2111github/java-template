...
public class Main {
    ...
    static void solve() throws IOException {
        int n = ri();
        dp = new Long[n+ 1][3][2][2];
        System.out.println(solve(n ,0, 0,0));
    }
    static long solve(int i,int p,int r,int e) {
        long mod = (long)1e9 + 7;
        if(i == 0) return (p >= 2 && r > 0 && e > 0)?1:0;
        if(dp[i][p][r][e] != null)  return dp[i][p][r][e];
        long ans = 0;
        for(char j = 'a';j <= 'z';j++) {
            int np = p , nr = r , ne = e;
            if(j == 'p') np++;
            if(j == 'r') nr++;
            if(j == 'e') ne++;
            ans += solve(i - 1 , Math.min(np  , 2),Math.min(nr,1),Math.min(ne,1));
            ans %= mod;
        }
        return dp[i][p][r][e] = ans;
    }
    ...
    // }
    // Hi Nakul, My name is Amit , I am a final Year student from NIT Mizoram , and a 4 star coder on codechef , and have a level of knight on Leetcode. Could you refer me for any opening for sde role in your organisation.
    // Hi hasMoreTokens  ,My name is Amit , I am a final Year student from NIT Mizoram , and a 4 star coder on codechef , and have a level of knight on Leetcode. I found an Opportunity in you organisation,Could you refer me for that. Link => https://remote.co/job/senior-full-stack-engineer-28-2/
